        ...
        public static class SetSettlementGizmosForOnline
        {
            ...
            public static void ModifyPost(ref IEnumerable<Gizmo> __result, Settlement __instance)
            {
                ...
                else if (__instance.Faction == Find.FactionManager.OfPlayer)
                {
                    ...
                    Command_Action command_Reformcaravan = new Command_Action
                    {
                        defaultLabel = "Reform Caravan",
                        defaultDesc = "Reform caravan and leave settlement.",
                        icon = ContentFinder<Texture2D>.Get("UI/Commands/MergeCaravans"),
                        action = delegate
                        {
                            Find.WindowStack.Add(new OW_MPLeaveMenu());
                        }
                    };
                    ...
                    if (Multiplayer.Client.Multiplayer.session != null && BooleanCache.isConnectedToServer && !BooleanCache.ishostingrtseserver)
                    {
                        gizmoList.Add(command_Reformcaravan);
                    }
                    if(Multiplayer.Client.Multiplayer.session != null)
                    {
                        gizmoList.RemoveAll(g => g.GetType().GetProperty("Label").GetValue(g)?.ToString() == "Form caravan");
                    }
                ...
                else
                {
                    ...
                    Command_Action command_RealtimeVisit = new Command_Action
                    {
                        ...
                        action = delegate
                        {
                            ...
                            Find.WindowStack.Add(new OW_SendVisitRequest("Send Visit Request To: " + FocusCache.focusedSettlement.ToString().Split('(')[0].Replace("'s Settlement", "").Replace("Settlement", "") + "?"));
                        }
                    };
