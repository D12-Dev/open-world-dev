...
{
    public class OW_MPTradeMenu : Window
    {
        ...
        public OW_MPTradeMenu()
        {
            FocusCache.tradeMenuInstance = this;

            TradeCache.wantedSilver = 0;
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            string a = Widgets.TextField(new Rect((inRect.width / 2) - (Text.CalcSize(silverWanted).x / 2) + 12f, silverDescriptionDif, 200f, 30f), TradeCache.wantedSilver.ToString());
            if (AcceptsInput && a.Length <= 7 && a.All(character => Char.IsDigit(character))) TradeCache.wantedSilver = int.Parse(a);
        ...
        {
            if (TradeCache.wantedSilver == 0) Find.WindowStack.Add(new OW_ErrorDialog("Silver quantity is needed to continue"));
            else Find.WindowStack.Add(new OW_MPConfirmTrade());
        }
