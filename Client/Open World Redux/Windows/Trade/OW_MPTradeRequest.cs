...
{
    public class OW_MPTradeRequest : Window
    {
        ...
        public OW_MPTradeRequest(string[] contents)
        {
            TradeCache.takeMode = TradeHandler.TakeMode.Trade;
            TradeCache.inTrade = true;

            List<string> tempContents = contents.ToList();
            senderTile = tempContents[0];
            tempContents.RemoveAt(0);
            requestedSilver = int.Parse(tempContents[0]);
            tempContents.RemoveAt(0);

            silverText += requestedSilver + " silver";
        ...
        {
            if (!RimworldHandler.CheckIfEnoughSilverInSettlement(requestedSilver))
            {
                string[] contentsReject = new string[] { senderTile };
            ...
            {
                string[] contentsAccept = new string[] { senderTile };
                Packet AcceptTradePacket = new Packet("AcceptTradePacket", contentsAccept);
                Network.SendData(AcceptTradePacket);

                RimworldHandler.TakeSilverFromSettlement(requestedSilver);
