...
namespace OpenWorldRedux
{
    public class OW_MPSpecialThanks : Window
    {
        ...
        private string patreonsListTitle = "Most supportive patreons";
        ...
        private string devotedListTitle = "Most devoted to the development process";
        private string[] devoteds = new string[]
        {
            "Saffue - Moderator",
            "Eragon - Moderator",
            "Warbrain - Artist",
            "BoostHungry - Github Contributor",
            "DarkIrata - Github Contributor",
            "ZephyrWarrior - Github Contributor",
            "Napstablook - Ex Moderator",
            "Ishiveki - Ex Moderator"
        };

        private string buddiesListTitle = "Best buddies that could be asked for";
        private string[] buddies = new string[]
        {
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            Rect rect1 = new Rect(rect.x, list1Dif, rect.width, 160f);
            GenerateList(rect1, patreons);

            Text.Font = GameFont.Small;
            Widgets.Label(new Rect(centeredX - Text.CalcSize(devotedListTitle).x / 2, list2TitleDif, Text.CalcSize(devotedListTitle).x, Text.CalcSize(devotedListTitle).y), devotedListTitle);
            Rect rect2 = new Rect(rect.x, list2Dif, rect.width, 160f);
            GenerateList(rect2, devoteds);

            Text.Font = GameFont.Small;
            Widgets.Label(new Rect(centeredX - Text.CalcSize(buddiesListTitle).x / 2, list3TitleDif, Text.CalcSize(buddiesListTitle).x, Text.CalcSize(buddiesListTitle).y), buddiesListTitle);
            Rect rect3 = new Rect(rect.x, list3Dif, rect.width, 160f);
            GenerateList(rect3, buddies);
